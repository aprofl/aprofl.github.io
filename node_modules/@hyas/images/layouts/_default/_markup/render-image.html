...
{{ end }}
{{ if $r }}
  {{ warnf "Image found: %s with MediaType: %s" $r.RelPermalink (or ($r.MediaType.SubType | default "unknown")) }}
  {{ $id := printf "h-rh-i-%d" .Ordinal }}
  {{ with .Attributes.id }}
    {{ $id = . }}
  {{ end }}
  {{ if and ($r.MediaType) (ne ($r.MediaType.SubType | default "") "gif") }}
  ...
  />
{{ else }}
  {{ warnf "Image resource not found for: %s" $u.Path }}
  <img
